...
class PayloadData {
  ...
  loadObject(payload){
    ...
    } else if(payload.data.hasOwnProperty(DB_FIELD_NAME.USER_ID)){
      this.userID = payload.data[DB_FIELD_NAME.USER_ID];
    }

    /** 이름이 있는 경우  */
    if(payload.hasOwnProperty(DB_FIELD_NAME.USER_NAME)){
      this.userName = payload[DB_FIELD_NAME.USER_NAME];
    } else if(payload.data.hasOwnProperty(DB_FIELD_NAME.USER_NAME)){
      this.userName = payload.data[DB_FIELD_NAME.USER_NAME];
    }
  }
