...
class RequestData{
  ...
  initData = async(data) =>{
    ...
    // if(1){ 
      ...
      // userInfo.viewAuth = await UserModel.trimJson(viewAuth);

      // this.userInfo = { ...userInfo };

      // return 0;
    // Cloud Foundry
    }else if(data.session != undefined && data.session.user != undefined){
    
      this.userInfo  = data.session.user;
    }
  ...
  beginTransaction = async () =>{
    ...
    await this.connection.setAutoCommit(false);
  }
  ...
  commit = async () => {
    console.log(`commit ^^^^^`);
    await this.connection.commit(function(err) {
      if (err) throw err;
   ...
   */
  rollback = async () => {
    await this.connection.rollback(function(err) {
    if (err) throw err;
