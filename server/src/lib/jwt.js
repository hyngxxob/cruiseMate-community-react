 ...
 * @returns {string} jwt 토큰
 */
const getRefreshJWTToken = (payload) => {
  ...
  const token = jwt.sign(
    ...
    process.env.SECRET_KEY,
    {
      expiresIn: process.env.REFRESH_EXPIRE_DAY,
      issuer: process.env.ISSUER,
    });
...
const checkSession = (req) => {
 ...
 // const requestData   = new RequestData(req.headers);
  // 응답 데이터
  const responseData = new ResponseData();
  responseData.setDataValue(DATA_FIELD_NAME.PAYLOAD, {data:{}});

  if((req.headers.origin.indexOf("localhost")  < 0 && req.headers.origin.indexOf("applicationstudio.cloud.sap") < 0  )
  
  &&(req.session.user == undefined || req.session.user.UUID == undefined)){
...
const checkRefreshToken = (req) => {
  ...
  const cooooook = req.cookies.refresh_token; 
